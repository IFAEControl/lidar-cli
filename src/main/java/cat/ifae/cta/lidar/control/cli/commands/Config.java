...
@CommandLine.Command(name = "config", mixinStandardHelpOptions = true, description = "See or change server configurations")
public class Config implements Runnable {
    private final static Logging _log = new Logging(Config.class);

    private ConfigGrpc.ConfigBlockingStub stub;

    @CommandLine.Option(names = "--get", description = "See server configuration")
    boolean is_get = false;

    @CommandLine.Option(names = "--check", description = "Check client configuration")
    ...
    public void run(){
        ...
        try {
            ...
            if(is_check) System.out.println("Configuration OK");
