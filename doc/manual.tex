...
Ping \textbf{lidar.ifae.es}. The server may take 10m to be completely operational, if after this time the the server is not operational, power off and on the lidar.

If the server is running, try to check if lidar service is also running. To do it check if port 81 is open. If not power off and on the lidar.
