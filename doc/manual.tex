...
\begin{document}
\title{LIDAR CLI User Manual - 0.0.1}
...
\begin{itemize}
	...
	\item \textbf{./licli operation ll  \texttt{-{}-}micro-init} (remember to check laser temperature and stop the hotwind when it reaches the proper temperature)
...
\begin{itemize}
	\item startup: Prepare the laser to shoot. Open the doors, power on all subsystems and move telescope to a good position (ie:above tilt).
	\item shutdown: Power off all subsystems and close the doors.
\end{itemize}
...
\begin{itemize}
	...
	\item micro-init: Micro initialization sequence. Prepare laser, arm and rump up all DACs. Note that if needed this command will start the hotwind but it will not monitor the temperature, the user must check the laser temperature and power off the hotwind if needed.
